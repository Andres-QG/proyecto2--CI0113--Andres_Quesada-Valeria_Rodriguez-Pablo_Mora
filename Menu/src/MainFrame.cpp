  ...
  : wxFrame(NULL, wxID_ANY, title)
{
  ...
  Connect(myID_GAME, wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(MainFrame::OnGame));
  wxButton *buttonInstructions = new wxButton(panel, myID_INSTRUCTIONS, wxT("Help"), wxPoint((width/3)*2, (height/5)*3));
  Connect(myID_INSTRUCTIONS, wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(MainFrame::OnInstructions));
  wxButton *buttonQuit = new wxButton(panel,wxID_EXIT,wxT("Quit"), wxPoint((width/3)*2,(height/5)*4));
  Connect(wxID_EXIT,wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(MainFrame::OnQuit));
  buttonQuit->SetFocus();
}
